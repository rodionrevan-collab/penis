...
func _initialize() -> void:
	...
	assert(progression.get_secret_mini_activities().size() == 3)
	assert(progression.get_secret_collection_total() == 3)
	assert(progression.get_secret_collection_count() == 0)
	assert(progression.get_available_secret_activities().size() == 3)
	var secret_activity_reward:Dictionary=progression.claim_secret_activity("pirate_navigation_secret")
	assert(secret_activity_reward["ok"] == true)
	assert(progression.is_secret_activity_completed("pirate_navigation_secret"))
	assert(progression.is_secret_collection_item_collected("pirate_compass"))
	assert(progression.get_secret_collection_count() == 1)
	assert(progression.claim_secret_activity("pirate_navigation_secret")["ok"] == false)
	var secret_cave_reward:Dictionary=progression.claim_secret_activity("cave_runes_secret")
	var secret_trade_reward:Dictionary=progression.claim_secret_activity("village_trade_secret")
	assert(secret_cave_reward["ok"] == true)
